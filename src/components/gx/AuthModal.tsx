...
export function AuthModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className={"gx-auth-modal" + (open ? " open" : "")}>
      <div className="gx-auth-modal__scrim" onClick={handleClose} />
      <div className="gx-auth-modal__card" role="dialog" aria-modal="true">
        <button type="button" className="gx-auth-modal__close" onClick={handleClose} aria-label={t("common.close")}>✕</button>
        ...
        <h3 className="gx-auth-modal__title">
          {mode === "2fa"
            ? t("auth.2fa_title")
            : mode === "signin"
            ? t("auth.signin_btn")
        ...
        <form className="gx-auth-modal__form" onSubmit={submit}>
          {mode === "2fa" ? (
            <>
              ...
              />
              <button type="submit" className="gx-auth-modal__submit" disabled={mfaCode.length !== 6}>
                {t("auth.2fa_verify_btn")}
              </button>
              <button type="button" className="gx-auth-modal__link" onClick={handleCancel2fa}>
