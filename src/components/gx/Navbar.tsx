...
export function Navbar() {
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (!t.closest(".account-wrap")) setAccountOpen(false);
    };
    document.addEventListener("click", onClick);
    return () => document.removeEventListener("click", onClick);
  }, [menuOpen, accountOpen]);

  const displayName = profile?.full_name || profile?.username || (profile?.email?.split("@")[0]) || "حسابي";
      ...
      <nav className="nav">
        <div className="wrap">
          ...
          <div className="nav-right">
            ...
            <button type="button" className="currency-pick" onClick={() => setCurrencyOpen(true)}>
              <span>{currency}</span>
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M6 9l6 6 6-6" /></svg>
            </button>
            <button type="button" className="icon-btn" onClick={cart.openDrawer} title="السلة">
              🛒
            ...
            <div className="menu-wrap">
              <button type="button" className="menu-btn" onClick={(e) => { e.stopPropagation(); setMenuOpen(v => !v); }}>
                ...
                <span className="btn-label">القائمة</span>
              </button>
              <div className={"menu-panel" + (menuOpen ? " open" : "")}>
                <div className="menu-section">
                  <div className="ms-title">الصفحات</div>
                  <MenuLink to="/" icon="🏠" label="الرئيسية" onClick={() => setMenuOpen(false)} />
                  <MenuLink to="/cart" icon="🛒" label="السلة" onClick={() => setMenuOpen(false)} />
                  <MenuLink to="/faq" icon="❓" label="الأسئلة الشائعة" onClick={() => setMenuOpen(false)} />
                  <MenuLink to="/policy" icon="🛡️" label="الضمان والاسترجاع" onClick={() => setMenuOpen(false)} />
                </div>
                ...
                <div className="menu-divider" />
                <div className="menu-section">
                  <div className="ms-title">تواصل معنا</div>
