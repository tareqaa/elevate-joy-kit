...
export const bulkCreateVariants = createServerFn({ method: "POST" })
  .inputValidator((data: { productId: string; rows: string }) => ({
  ...
  .handler(async ({ data }) => {
    ...
    const variants = lines.map(line => {
      const [labelAr, labelEn, price, oldPrice, cartId] = line.split(",").map(s => s.trim());
      return {
        product_id: data.productId,
        label_ar: labelAr,
        label_en: labelEn || labelAr,
        price_jod: Number(price) || 0,
        old_price_jod: oldPrice ? Number(oldPrice) : null,
        cart_id: cartId || `v-${Math.random().toString(36).slice(2, 7)}`,
        is_active: true,
      };
    });
