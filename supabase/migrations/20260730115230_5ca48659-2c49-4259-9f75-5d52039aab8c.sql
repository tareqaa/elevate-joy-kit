-- 1) Track store credit (refund balance) spent on each order.
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS credit_used_jod numeric NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS credit_refunded_jod numeric NOT NULL DEFAULT 0;

-- Backfill from the existing store-credit ledger.
UPDATE public.orders o
   SET credit_used_jod = s.spent
  FROM (
    SELECT order_id, SUM(-amount_jod) AS spent
      FROM public.store_credit_transactions
     WHERE kind = 'spend' AND order_id IS NOT NULL AND amount_jod < 0
     GROUP BY order_id
  ) s
 WHERE s.order_id = o.id
   AND COALESCE(o.credit_used_jod, 0) = 0;

-- 2) Idempotent, proportional store-credit return (mirrors refund_order_coins).
CREATE OR REPLACE FUNCTION public.refund_order_credit(_order_id uuid, _ratio numeric)
RETURNS numeric
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE o public.orders%ROWTYPE; target numeric; delta numeric; after_bal numeric; r numeric;
BEGIN
  ...
  UPDATE public.profiles
     SET store_credit_jod = COALESCE(store_credit_jod, 0) + delta
   WHERE id = o.user_id
  ...
  VALUES (o.user_id, delta, after_bal, 'refund',
          'إرجاع رصيد المتجر للطلب ' || o.order_number, o.id);

  UPDATE public.orders SET credit_refunded_jod = target, updated_at = now() WHERE id = o.id;
  RETURN delta;
END $function$;

REVOKE EXECUTE ON FUNCTION public.refund_order_credit(uuid, numeric) FROM PUBLIC, anon, authenticated;

-- 3) Cancelling an order returns BOTH coins and store credit.
...
BEGIN
  IF NEW.status = 'cancelled' AND OLD.status IS DISTINCT FROM 'cancelled' AND NEW.user_id IS NOT NULL THEN
    ...
    IF COALESCE(NEW.credit_used_jod, 0) > 0 THEN
      PERFORM public.refund_order_credit(NEW.id, 1);
    END IF;
  END IF;
  RETURN NEW;
END $function$;

-- 4) Refunds return the same proportion of store credit that was spent.
...
BEGIN
  ...
  IF o.user_id IS NOT NULL THEN
    ...
    -- Return what the customer SPENT on this order (proportional, once each).
    _coins_returned := public.refund_order_coins(o.id, _ratio);
    _credit_returned := public.refund_order_credit(o.id, _ratio);
...
-- 5) Admin order-amount edits must treat store credit as its own bucket.
CREATE OR REPLACE FUNCTION public.admin_set_order_amounts(_order_id uuid, _subtotal_jod numeric, _coins_used bigint, _reason text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
DECLARE
  o public.orders%ROWTYPE;
  _email TEXT;
  _coupon_disc NUMERIC; _coins_disc NUMERIC; _credit_disc NUMERIC; _max_coins BIGINT;
...
BEGIN
  ...
  _credit_disc := GREATEST(COALESCE(o.credit_used_jod, 0) - COALESCE(o.credit_refunded_jod, 0), 0);
  _coupon_disc := GREATEST(COALESCE(o.discount_jod,0) - COALESCE(o.coins_discount_jod,0) - _credit_disc, 0);
  IF _coupon_disc > _subtotal_jod THEN _coupon_disc := _subtotal_jod; END IF;
  IF _credit_disc > GREATEST(_subtotal_jod - _coupon_disc - _coins_disc, 0) THEN
    _credit_disc := GREATEST(_subtotal_jod - _coupon_disc - _coins_disc, 0);
  END IF;
  _total := GREATEST(_subtotal_jod - _coupon_disc - _coins_disc - _credit_disc, 0);
  ...
  UPDATE public.orders
     SET subtotal_jod = _subtotal_jod,
         ...
         discount_jod = _coupon_disc + _coins_disc + _credit_disc,
         total_jod = _total,
         paid_jod = _total,
         admin_notes = COALESCE(admin_notes,'') ||
           CASE WHEN COALESCE(admin_notes,'') = '' THEN '' ELSE E'\n' END ||
  ...
  VALUES (auth.uid(), _email, 'order.amounts_edited', 'order', o.id::text,
          jsonb_build_object('order_number', o.order_number, 'subtotal', _subtotal_jod,
                             ...
                             'credit_used', _credit_disc, 'total', _total, 'reason', TRIM(_reason)));

  RETURN jsonb_build_object('ok', true, 'total', _total, 'coins_used', _coins_used,
                            'coins_delta', _delta, 'coins_discount_jod', _coins_disc,
                            'credit_used_jod', _credit_disc);
END $function$;

REVOKE EXECUTE ON FUNCTION public.admin_set_order_amounts(uuid, numeric, bigint, text) FROM PUBLIC, anon;
